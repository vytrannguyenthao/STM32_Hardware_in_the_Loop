...
Hệ thống HIL hỗ trợ mô phỏng các thiết bị ngoại vi giao tiếp I2C nhằm phục vụ quá trình kiểm thử firmware của DUT. Trong phạm vi đề tài, hai thiết bị I2C được lựa chọn để mô phỏng là EEPROM và RTC do đây là các ngoại vi phổ biến trong nhiều hệ thống nhúng thực tế.

Trong hệ thống, DUT đóng vai trò master I2C, trong khi HIL hoạt động ở chế độ slave và phản hồi các yêu cầu truy cập từ DUT. Thay vì sử dụng các linh kiện phần cứng riêng biệt, các thiết bị ngoại vi được mô phỏng hoàn toàn bằng phần mềm trên vi điều khiển STM32 của hệ thống HIL.
...
Cơ chế này cho phép hệ thống HIL quản lý linh hoạt nhiều thiết bị I2C khác nhau mà không phụ thuộc trực tiếp vào số lượng địa chỉ slave phần cứng được hỗ trợ bởi vi điều khiển.

\subsubsection{Cơ chế xử lý callback và định tuyến thiết bị}

Hệ thống HIL sử dụng cơ chế ngắt và callback của thư viện HAL để xử lý các sự kiện giao tiếp I2C. Vi điều khiển STM32 hoạt động ở chế độ slave listen mode và liên tục chờ yêu cầu truy cập từ DUT.
...
Thiết bị EEPROM được mô phỏng nhằm phục vụ kiểm thử các chức năng đọc và ghi dữ liệu không mất nguồn của DUT. Toàn bộ dữ liệu EEPROM được lưu trữ trong vùng nhớ RAM của vi điều khiển HIL nhằm tăng tốc độ truy cập và đơn giản hóa quá trình kiểm thử.
...
Trong quá trình đọc dữ liệu, hệ thống HIL truy xuất dữ liệu trực tiếp từ vùng nhớ mô phỏng và phản hồi lại DUT thông qua giao tiếp I2C. Sau mỗi byte dữ liệu được truyền thành công, địa chỉ truy cập sẽ tự động tăng để mô phỏng cơ chế đọc liên tục của EEPROM thực tế.

\subsubsection{Mô phỏng RTC}

Ngoài EEPROM, hệ thống HIL còn hỗ trợ mô phỏng thiết bị RTC nhằm kiểm thử các chức năng liên quan đến thời gian thực của DUT.
...
Để đảm bảo khả năng hoạt động tương tự thiết bị thực, hệ thống HIL sử dụng bộ định thời nội của vi điều khiển để cập nhật giá trị thời gian theo chu kỳ 1 giây.
...
Khi DUT thực hiện thao tác ghi dữ liệu lên RTC, byte đầu tiên được sử dụng để xác định địa chỉ thanh ghi truy cập. Các byte dữ liệu tiếp theo sẽ được ghi tuần tự vào các thanh ghi tương ứng. Trong trường hợp DUT thực hiện thao tác đọc, hệ thống HIL sẽ phản hồi dữ liệu từ các thanh ghi RTC và tự động tăng con trỏ thanh ghi sau mỗi lần truyền thành công nhằm mô phỏng cơ chế truy cập liên tục của DS1307 thực tế.
...
Hệ thống HIL hỗ trợ chức năng điều khiển và giám sát các chân GPIO nhằm phục vụ kiểm thử các tín hiệu số của DUT. Các chân GPIO được cấu hình sẵn theo hai chế độ input và output tùy theo mục đích sử dụng trong hệ thống.
...
Hệ thống HIL hỗ trợ tạo tín hiệu PWM nhằm phục vụ kiểm thử các chức năng điều khiển và đo lường của DUT. Các tín hiệu PWM được tạo bằng bộ định thời phần cứng của vi điều khiển STM32 nhằm đảm bảo khả năng tạo tín hiệu ổn định và có thể cấu hình động trong thời gian thực.
