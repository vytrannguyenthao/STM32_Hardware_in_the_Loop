...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{image/robot_framework_structure.png}
...
Ưu điểm lớn nhất của Robot Framework là khả năng chuẩn hóa cấu trúc test case và tách biệt rõ ràng giữa phần logic kiểm thử với phần xử lý giao tiếp phần cứng. Các thao tác phức tạp như gửi lệnh UART, cấu hình EEPROM giả lập, điều khiển RTC, đọc dữ liệu HIL Collector hoặc kiểm tra tín hiệu PWM có thể được xây dựng trong các thư viện Python tự phát triển, sau đó đóng gói thành các keyword đơn giản để tái sử dụng trong nhiều test case khác nhau. Điều này giúp hệ thống vừa giữ được tính linh hoạt của Python, vừa đảm bảo khả năng quản lý tập trung và dễ bảo trì.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/hil_robot_structure.png}
...
Robot Framework đóng vai trò điều phối trung tâm, thực thi tuần tự các test case và gọi các keyword tương ứng trong các thư viện kiểm thử. Đối với hệ thống HIL Generator và HIL Collector, các keyword được xây dựng sẵn và lưu trữ trong thư viện hệ thống. Các keyword này tương ứng trực tiếp với toàn bộ tập lệnh được hỗ trợ bởi HIL Generator và HIL Collector, cho phép thực hiện các thao tác gửi lệnh, nhận phản hồi và cấu hình môi trường mô phỏng. Bên cạnh đó, hệ thống cũng cung cấp thêm các keyword tiện ích phục vụ kiểm thử như kiểm tra trạng thái thiết bị, so sánh giá trị phản hồi với giá trị mong đợi và xác nhận kết quả PASS/FAIL.
...
Tầng giao tiếp vật lý sử dụng thư viện \texttt{pyserial} để kết nối với DUT, HIL Generator và HIL Collector thông qua cổng USB serial. Thư viện này chịu trách nhiệm thực hiện truyền nhận dữ liệu, đồng bộ quá trình kiểm thử và đảm bảo luồng giao tiếp ổn định giữa các thiết bị trong suốt quá trình thực thi test case.
...
Dựa trên kiến trúc hệ thống kiểm thử đã xây dựng cùng tập các keyword hỗ trợ cho HIL Generator và HIL Collector, bước tiếp theo là thiết kế các kịch bản kiểm thử cụ thể cho từng chức năng ngoại vi của DUT. Các keyword được sử dụng như những khối chức năng cơ bản để xây dựng quy trình kiểm thử hoàn chỉnh, bao gồm thiết lập môi trường kiểm thử, điều khiển DUT và HIL, thu thập dữ liệu phản hồi cũng như đánh giá kết quả theo tiêu chí PASS/FAIL.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/connection_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{image/gpio_output_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{image/gpio_input_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/adc_test_flowchart.png}
...
Trong bài kiểm thử này, DUT đóng vai trò chủ động phát sinh tín hiệu analog dưới dạng sóng sine và sóng tam giác với tần số xác định trước. Hệ thống HIL Collector được sử dụng để thu thập trực tiếp tín hiệu analog tại chân DAC, sau đó dữ liệu được xử lý và phân tích bằng thuật toán FFT nhằm xác minh đặc tính của dạng sóng đầu ra.

Quy trình kiểm thử tổng quát bao gồm các bước sau:

\begin{itemize}
    \item Bước 1: Thiết lập môi trường kiểm thử bằng cách kết nối HIL Generator, HIL Collector, cấp nguồn cho DUT và thiết lập kết nối với DUT.
    \item Bước 2: DUT thực hiện phát tín hiệu sóng sine với tần số xác định trước.
    \item Bước 3: HIL Collector thu thập dữ liệu tín hiệu analog tại chân DAC và hệ thống thực hiện phân tích để xác minh tín hiệu có đúng dạng sóng sine và đúng tần số mong đợi hay không.
    \item Bước 4: DUT tiếp tục phát tín hiệu sóng tam giác với tần số xác định trước.
    \item Bước 5: HIL Collector tiếp tục thu thập dữ liệu và hệ thống thực hiện phân tích để xác minh tín hiệu có đúng dạng sóng tam giác và đúng tần số mong đợi hay không.
    \item Bước 6: Kết thúc môi trường kiểm thử bằng cách ngắt kết nối DUT, tắt nguồn DUT, ngắt kết nối HIL Collector và HIL Generator.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/dac_test_flowchart.png}
    \caption{Lưu đồ giải thuật tổng quát kiểm tra chức năng DAC}
    \label{fig:dac_test_flowchart}
\end{figure}

Đối với bài kiểm tra sóng sine, dữ liệu analog thu được từ HIL Collector sẽ được chuyển đổi sang miền tần số bằng thuật toán FFT. Hệ thống tiến hành xác định đỉnh năng lượng chính và tính tỷ lệ năng lượng giữa thành phần tín hiệu chính với phần nhiễu còn lại trong phổ tần.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/verify_triangle_wave_flowchart.png}
...
Trong bài kiểm thử này, DUT đóng vai trò chủ động phát sinh tín hiệu PWM tại chân đầu ra được chỉ định. Hệ thống HIL Collector được sử dụng để thu thập trực tiếp tín hiệu số tại chân PWM, sau đó dữ liệu được xử lý để xác định tần số thực tế và độ rộng xung nhằm so sánh với giá trị mong đợi.
...
\begin{itemize}
    ...
    \item Bước 5: HIL Collector thu thập dữ liệu tín hiệu số tại chân PWM trong khoảng thời gian xác định.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.35\textheight]{image/pwm_test_flowchart.png}
    \caption{Lưu đồ giải thuật tổng quát kiểm tra chức năng PWM}
    \label{fig:pwm_test_flowchart}
\end{figure}

Trong bài kiểm thử này, HIL Collector thực hiện thu thập dữ liệu số tại chân PWM và tách riêng kênh digital tương ứng để phân tích. Thuật toán kiểm tra được xây dựng dựa trên phương pháp phát hiện cạnh lên nhằm xác định chu kỳ tín hiệu.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/uart_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/can_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{image/eeprom_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/rtc_test_flowchart.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{image/spi_flash_test_flowchart.png}
