...
Sau khi xây dựng kiến trúc tổng thể của hệ thống kiểm thử tự động bằng Robot Framework, bước tiếp theo là thiết kế các kịch bản kiểm thử cụ thể cho từng chức năng ngoại vi của DUT. Mỗi kịch bản kiểm thử được xây dựng nhằm xác minh tính đúng đắn của firmware khi thực hiện giao tiếp với các thiết bị ngoại vi được mô phỏng bởi hệ thống HIL.

Các bài kiểm thử được tổ chức tương ứng với từng nhóm chức năng như GPIO, PWM, UART, I2C, SPI, CAN và các ngoại vi mở rộng như eeprom, rtc và bộ nhớ flash. Mỗi test case bao gồm đầy đủ các bước từ khởi tạo môi trường kiểm thử, cấu hình hệ thống HIL, điều khiển DUT thực hiện tác vụ cần kiểm tra, thu thập dữ liệu phản hồi cho đến đánh giá kết quả cuối cùng theo tiêu chí PASS/FAIL đã được xác định trước.
