...
\section{Các keyword hỗ trợ trong hệ thống kiểm thử}

Để hỗ trợ xây dựng các kịch bản kiểm thử tự động một cách trực quan và có khả năng tái sử dụng cao, hệ thống cung cấp một tập hợp các keyword phục vụ cho việc điều khiển HIL và Logic Analyzer. 

Các keyword này được xây dựng dưới dạng thư viện Python và tích hợp trực tiếp vào Robot Framework nhằm hỗ trợ thực hiện các thao tác như điều khiển nguồn DUT, mô phỏng ngoại vi, gửi và nhận dữ liệu truyền thông, thu thập tín hiệu số và tín hiệu analog cũng như đánh giá kết quả kiểm thử tự động.

Bảng \ref{tab:hil_keywords} và Bảng \ref{tab:la_keywords} trình bày một số keyword tiêu biểu được hỗ trợ trong hệ thống kiểm thử.
...
Các keyword trên đóng vai trò là lớp giao tiếp trung gian giữa Robot Framework với hệ thống HIL và Logic Analyzer. Thay vì thao tác trực tiếp với các lệnh điều khiển mức thấp, người phát triển chỉ cần sử dụng các keyword tương ứng để xây dựng các test case theo dạng mô tả hành vi kiểm thử.

Nhờ đó, các kịch bản kiểm thử trở nên trực quan, dễ đọc, dễ bảo trì và thuận tiện cho việc mở rộng hệ thống trong tương lai.
