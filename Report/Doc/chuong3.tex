...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{image/hw_block_diagram.jpg}
...
\begin{itemize}
    ...
    \item \textbf{Khối HIL (Generator và Collector):}
        \begin{itemize}
            \item \textbf{Generator:} Tập trung giao tiếp, xử lý dữ liệu với DUT bằng cách ngoại vi thông dụng như UART, SPI, I2C, CAN, GPIO, DAC. Tạo các dạng sóng như sóng sine, sóng tam giác, PWM. Đồng thời mô phỏng một số phần cứng phổ biến như EEPROM, Flash, RTC.
            \item \textbf{Collector:} Tập trung xử lý dữ liệu analog và digital từ DUT, hoạt động như một logic analyzer hoặc oscilloscope và hiển thị dạng sóng lên PC qua giao tiếp USB.
        \end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{image/hw_detail.jpg}
...
\begin{itemize}
    ...
    \item \textbf{HIL Generator:} Sử dụng vi điều khiển STM32F407VET6 hoạt động với nguồn 3.3V, có tích hợp các ngoại vi như CAN, DAC, UART, SPI,... phù hợp để mô phỏng các phần cứng và giao tiếp với DUT, PC.
    \item \textbf{HIL Collector:} Sử dụng module Raspberry Pi Pico, hoạt động với nguồn 3.3V thực hiện các chức năng để thu thâp, xử lý tín hiệu analog và digital từ DUT.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{image/pcb.jpg}
    \caption{PCB của hệ thống HIL}
    \label{fig:pcb}
\end{figure}
Những khối chức năng chính trên chính trên hệ thống được thiết kế chi tiết như sau:
\subsection{Khối nguồn}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{image/source_hil.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{image/source_dut.jpg}
...
Hệ thống được cấp nguồn với điện áp đầu vào 5V DC qua jack DC và được hạ xuống 3.3V để cung cấp cho khối HIL gồm Generator và Collector và các module khác.\\
\begin{figure}[H]
    \centering
...
Nguồn cung cấp cho DUT và các module liên quan cũng được hạ áp từ nguồn 5V đầu vào. Để HIL có thể điều khiển nguồn cho DUT, cần kết hợp mạch hạ áp với khóa điện tử dùng MOSFET.\\
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.32\textwidth]{image/power.jpg}
    \caption{Khối nguồn thực tế}
    \label{fig:power}
\end{figure}

\subsection{Khối HIL Generator}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/hil_block.jpg}
    \caption{Sơ đồ mạch của HIL Generator}
    \label{fig:hil_block}
\end{figure}
...
% \begin{table}[H]
% \centering
% \caption{Các ngoại vi và chân tín hiệu trong hệ thống HIL}
% \renewcommand{\arraystretch}{1.2}
% \begin{tabularx}{\textwidth}{|l|c|X|} 
% \hline
% \textbf{Ngoại vi / Tín hiệu} & \textbf{Chân tín hiệu} & \textbf{Mô tả} \\ 
% \hline
% Power Control & PE14 & Điều khiển nguồn của DUT \\ 
% \hline
% Power Status Input & PD1 & Đọc trạng thái nguồn của DUT \\ 
% \hline
% BOOT CTRL & PD0 & Kích hoạt chế độ Boot 1 để nạp chương trình bằng USB \\ 
% \hline
% SPI3 & \parbox[t]{3cm}{\centering PC10 (SCK), PC11 (MISO), PC12 (MOSI), PA15 (NSS) \vspace{2pt}} & Mô phỏng SPI Flash \\ 
% \hline
% I2C1 & \parbox[t]{3cm}{\centering PB6 (SCL), PB7 (SDA) \vspace{3pt}} & Mô phỏng I2C EEPROM \\ 
% \hline
% USART2 & \parbox[t]{3cm}{\centering PD5 (TX), PD6 (RX) \vspace{3pt}} & Giao tiếp UART \\ 
% \hline
% CAN1 & \parbox[t]{3cm}{\centering PB9 (TX), PB8 (RX) \vspace{3pt}} & Giao tiếp CAN bus \\ 
% \hline
% PWM & PA1, PA2, PA3 & Xuất tín hiệu PWM điều khiển \\ 
% \hline
% DAC & PA5 & Xuất tín hiệu analog (DAC) \\ 
% \hline
% Waveform Analog & PA4 & Phát tín hiệu dạng sóng (sin, tam giác) \\ 
% \hline
% ADC & PA0 & Đọc tín hiệu analog từ DUT \\ 
% \hline
% GPIO Input & PE0 -- PE13 & Đọc tín hiệu số từ DUT \\ 
% \hline
% GPIO Output & \parbox[t]{3cm}{\centering PB0 -- PB5, PB10 -- PB15 \vspace{3pt}} & Xuất tín hiệu số điều khiển DUT \\ 
% \hline
% \end{tabularx}
% \label{tab:hil_peripherals_pins}
% \end{table}

\subsection{Khối HIL Collector}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{image/hw_pi.jpg}
    \caption{Sơ đồ mạch của HIL Collector}
    \label{fig:hw_pi}
\end{figure}
...
thực hiện các test case liên quan đến việc xử lý tín hiệu như kiểm tra tần số, độ rộng xung,\dots
...
% \begin{tabularx}{\textwidth}{|l|c|X|} 
% \hline
% \textbf{Tín hiệu} & \textbf{Chân tín hiệu} & \textbf{Mô tả} \\ 
% \hline
% Digital Channel & GPIO2 -- 8 & Thu thập tín hiệu số từ DUT \\ 
% \hline
% Analog Channel & GPIO26 & Thu thập tín hiệu analog từ DUT \\
% \hline
% UART & GPIO0 (TX), GPIO1 (RX) & Debug UART \\
% \hline
% \end{tabularx}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \vspace{-10pt}
    \caption{Sơ đồ mạch giao tiếp CAN của HIL và DUT}
\end{figure}

HIL Generator và DUT đều sử dụng giao tiếp CAN để trao đổi dữ liệu. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{image/header_hil.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{image/header_dut.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{image/connector.jpg}
    \caption{Khối giao tiếp và connector mở rộng thực tế}
    \label{fig:connector}
\end{figure}
